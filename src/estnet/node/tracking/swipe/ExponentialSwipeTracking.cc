...
#include "ExponentialSwipeTracking.h"
...
Define_Module(ExponentialSwipeTracking);

inet::Quaternion ExponentialSwipeTracking::getNewOrientation() {
    ...
    ratioOfPassedPassTime = applyScaling(ratioOfPassedPassTime);
...
double ExponentialSwipeTracking::applyScaling(double ratio) {
    double rtn = 0;
    if (ratio <= 0.5) {
