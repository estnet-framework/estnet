...
cJulian::cJulian(int year,               // i.e., 2004
    ...
    int N;
    int F1 = static_cast<int>((275.0 * mon) / 9.0);
    int F2 = static_cast<int>((mon + 9.0) / 12.0);
...
void cJulian::Initialize(int year, double day) {
    ...
    double NewYears = static_cast<int>(365.25 * year)
            + static_cast<int>(30.6001 * 14) + 1720994.5 + B; // 1720994.5 = Oct 30, year -1
...
void cJulian::getComponent(int *pYear, int *pMon /* = nullptr */,
        double *pDOM /* = nullptr */) const {
    ...
    double A = Z + 1 + alpha - static_cast<int>(alpha / 4.0);
    double B = A + 1524.0;
...
//       Nov/Dec 1995
inetu::deg cJulian::toGMST() const {
    ...
    return inetu::rad((TWOPI * (GMST / SEC_PER_DAY)));
}
...
inetu::deg cJulian::toLMST(inetu::deg lon) const {
    return inetu::rad(fmod(inetu::rad(toGMST() + lon).get(), TWOPI));
}
...
time_t cJulian::toTime() const {
    ...
    int secs = static_cast<int>((dblDay * SEC_PER_DAY) + 0.5);
...
bool operator<(const cJulian& lhs, const cJulian& rhs) {
    return lhs.m_Date < rhs.m_Date;
}

cJulian operator+(const cJulian& lhs, const cJulian& rhs) {
    return cJulian(lhs.m_Date + rhs.m_Date);
}
