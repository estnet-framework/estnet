...
//////////////////////////////////////////////////////////////////////////////class ESTNET_API cJulian {
public:
  cJulian();
  explicit cJulian(double time);          // Create from double
  explicit cJulian(time_t time);          // Create from time_t
  ...
  inetu::deg toGMST() const;           // Greenwich Mean Sidereal Time
  inetu::deg toLMST(inetu::deg lon) const; // Local Mean Sideral Time
  ...
  void getComponent(int *pYear, int *pMon = nullptr,
  ...
  friend bool operator<(const cJulian& lhs, const cJulian& rhs);
  friend cJulian operator-(const cJulian& lhs, const cJulian& rhs);
  friend cJulian operator+(const cJulian& lhs, const cJulian& rhs);
