...
//omnetpp::simsignal_t DirectionalScalarBackgroundNoise::antNoise = registerSignal("antNoise");
...
const double DirectionalScalarBackgroundNoise::computeEarthInFOV(
    ...
    //geting some basic values from the input variables
    ...
    if (h_sat < (100000 + R_E)) {
        /*if the satellite is close to earth the curvature doesn't play a significant role and some
         ...
         of a flat earth*/
        double phi_hori = phi_sat - (0.5 * PI); // angle by wich the satellite is off the horizon
    ...
    //earth center, the satellite and the horizon form a right trianlge
    ...
    if (beamWidth >= 360) {
        // the antenna is omnidirectional, this simplifies the calclation
        double A_FOV = 4 * PI * (d_hori * d_hori); // area of a sphere that intersects with the horizon
        double A_E = 2 * PI * (d_hori * d_hori) * (1 - std::cos(alpha_hori)); // section of A_FOV obscured by the earth
    ...
    /*the center of the earth, the center of the cross section and the two points where the two circles intersect form
     multiple right triangles. Using the law of cosinces we can determine the angles between the line connecting the
...
const inet::physicallayer::INoise* DirectionalScalarBackgroundNoise::computeNoise(
    ...
    //workaround for emit() not being declared as const
    //const_cast<DirectionalScalarBackgroundNoise *>(this)->emit(antNoise, 1);
    //this->emit(antNoise, P_N_earth * EarthInFOV); // emit a signal for plotting

    //inet::units::values::W powerChange = power + P_N_antenna;
